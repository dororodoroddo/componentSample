...
---
    ...
    - 깔끔하고 좋은 코드를 작성하는 것도 매우 중요하나 바닐라 JS로 진행하는 과제이고 깔끔한 코드는 주변 사람과 함께 맞추어 
    나가는 것이지만 자바스크립트와 라이브러리의 구조를 이해하고 어려운 구조를 구현해내는 것은 각자의 역량으로 해내는 것이기 
    때문에 반응형 컴포넌트와 전역 스토어 구현에 집중하여 개발하였습니다.
